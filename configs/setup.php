...
// Avoid setup page to be cached (only if not behind proxy)
// Avoid setup page to be cached
// These headers can conflict with Nginx caching
...
$(document).ready(function() {
	...
	// Auto-generate secretkey if empty
	if ($('#opt_secretkey').val() === '') {
		var randomChars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
		var randomPart = '';
		for (var i = 0; i < 48; i++) {
			randomPart += randomChars.charAt(Math.floor(Math.random() * randomChars.length));
		}
		$('#opt_secretkey').val('NOXRL8' + randomPart);
	}
	
	// Auto-set template to bootstrap
	$('#opt_template_used').val('bootstrap');
