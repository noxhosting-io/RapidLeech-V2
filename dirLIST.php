...
if(!empty($_GET['folder']))
	$options['download_dir'] .= $url_folder."/";
...
?>
<title>dirLIST - <?PHP if(empty($url_folder)) echo "Index of: home/"; else echo "Index of: home/".$url_folder."/"; ?></title>
<style type="text/css">
<!--
...
if($listing_mode == 0)
	$folder_exists = (!is_dir($options['download_dir'])) ? false : true; //HTTP
elseif($listing_mode == 1 && PHP_VERSION >= 5)
	$folder_exists = (!is_dir('ftp://'.$ftp_username.':'.$ftp_password.'@'.$ftp_host.$options['download_dir'])) ? false : true; //FTP
...
//This is a VERY important security feature. It prevents people from browsing directories above $options['download_dir'] and any excluded folders. Edit this part at your own risk
...
$dir_content = get_dir_content($options['download_dir']);
...
if($view_mode == 1)
{
	if($show_folder_size_http == 1 && $listing_mode == 0)
		foreach($folders['name'] as $key => $val)
			$folders['size'][$key] = folder_size($options['download_dir'].$folders['name'][$key]);
