...
{
	header("Location: ../dirLIST.php?folder=".$_POST['folder']."&err=".base64_encode("size"));
	exit;
}
...
{
	header("Location: ../dirLIST.php?folder=".$_POST['folder']."&err=".base64_encode("nofile"));
	exit;
}
...
if($_POST['submit'] == $local_text['upload'])
{
	...
	$new_path = '../'.$options['download_dir'].$folder.$file_name;
	
	if(in_array(strtolower(strrchr($file_name, ".")), $banned_file_types))
	{
		header("Location: ../dirLIST.php?folder=".$_POST['folder']."&err=".base64_encode("upload_banned"));
		exit;
	}
	...
	while(is_file($new_path))
	{
		...
		$new_path = '../'.$options['download_dir'].$folder.$file_name_comp.'['.$same_file_counter.']'.$file_ext_comp;																																										
		$same_file_counter++;
	}
	
	if(move_uploaded_file($_FILES['file']['tmp_name'], $new_path))
	{
		header("Location: ../dirLIST.php?folder=".$_POST['folder']);
		exit;
	}	
	else
	{
		header("Location: ../dirLIST.php?folder=".$_POST['folder']."&err=".base64_encode("upload_error"));
		exit;
	}
}
else
