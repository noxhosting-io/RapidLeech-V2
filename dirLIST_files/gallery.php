...
$folder = '../'.$options['download_dir'].base64_decode($_GET['folder']);
...
foreach($content['files']['name'] as $key => $val)
{
	if(in_array(strtolower(strrchr($val, '.')), array('.jpg', '.jpeg', '.png', '.gif')))
	{
		...
		@$js_images_download_link .= '\''.base64_encode($options['download_dir'].base64_decode($_GET['folder']).'/'.$val).'\', ';
	}
}
...
<table width="695" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="543"><span class="header_text">dirLIST - Gallery</span></td>
